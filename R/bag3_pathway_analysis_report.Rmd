...
rownames(co_ip_genes_abun) <- NULL

eturkesRutils::datatable(co_ip_genes_abun) %>%
  DT::formatRound(c(2, 3, 4)) %>% DT::formatStyle(cols[1], `text-align` = "center")
```

## Input into g:Profiler {.tabset}
...
```{r}
gprofiler_spec <- gprofiler2::gost(
  co_ip_genes_spec$Genes, organism = "rnorvegicus", ordered_query = TRUE, exclude_iea = TRUE
)

gprofiler2::gostplot(gprofiler_spec, capped = TRUE, interactive = TRUE)
```
