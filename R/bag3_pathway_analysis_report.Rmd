...
co_ip_cols <- c("Gene", "BAG3 Specificity", "Abundance Ratio",
...
```{r}
co_ip_genes_spec <- co_ip_genes[order(-co_ip_genes[[co_ip_cols[2]]], co_ip_genes[[co_ip_cols[6]]]), ]
...
eturkesRutils::datatable(co_ip_genes_spec) %>% # From my own package, provides custom formatting.
  formatRound(c(2, 3, 4)) %>% formatStyle(co_ip_cols[1], `text-align` = "center")
```

### Abundance of Protein

```{r}
co_ip_genes_abun <- co_ip_genes[order(-co_ip_genes[[co_ip_cols[5]]], -co_ip_genes[[co_ip_cols[6]]]), ]
...
Tables are sorted by p-value and secondary sorted by Documents with BAG3.
...
eturkesRutils::datatable(co_occur) %>% # From my own package, provides custom formatting.
